...
async function handleSubmit(
  ...
  try {
    const transformedData: CreateFestivalInput = {
      ...
      dateStart: data.dateStart instanceof Date ? data.dateStart : new Date(String(data.dateStart)),
      dateEnd: data.dateEnd instanceof Date ? data.dateEnd : new Date(String(data.dateEnd)),
...
<template>
  <ModalForm :form-config="CREATE_FESTIVAL_FORM" :schema="createFestivalInputSchema" title="Create Festival"
    submit-label="Create" @submit="handleSubmit">
    <template #trigger>
      <PlusCircleIcon
