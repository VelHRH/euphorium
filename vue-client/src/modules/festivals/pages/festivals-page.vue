...
<template>
  <div class="min-h-screen bg-linear-to-br from-background via-background to-muted/20 p-6">
    <div class="max-w-6xl mx-auto mb-12 text-center space-y-4">
      ...
      <div class="w-32 h-1 bg-linear-to-r from-neon-purple via-neon-pink to-neon-cyan mx-auto rounded-full"></div>
    </div>
    ...
    <div class="max-w-6xl mx-auto">
      <Accordion type="single" collapsible class="space-y-6">
        <AccordionItem v-for="(edge, index) in festivalsList" :key="edge.node.id" :value="edge.node.id"
          ...
          <AccordionTrigger
            ...
            <div class="absolute bottom-4 left-4 w-2 h-2 bg-neon-cyan rounded-full animate-pulse delay-300"></div>
            <div
            ...
            <div class="flex-1 text-left">
              <h3 class="text-2xl font-semibold group-hover:text-neon-purple transition-colors duration-300">
                {{ edge.node.name }}
              </h3>
            ...
            <div class="hidden md:flex gap-2 opacity-60 group-hover:opacity-100 transition-opacity duration-300">
              <div v-for="(imgPath, imgIndex) in edge.node.imgPaths?.slice(0, 3)" :key="imgIndex"
                ...
                <Image :src="imgPath" :alt="`${edge.node.name} preview ${imgIndex + 1}`"
