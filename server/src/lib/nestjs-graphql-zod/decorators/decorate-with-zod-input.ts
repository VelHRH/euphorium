 ...
 * @return {F}
 */
export function decorateWithZodInput<T extends AnyZodObject, F extends Fn = Fn>(
  originalFunction: F,
  input: T,
  ...
  return function _modelWithZod(this: any, ...args: Parameters<F>) {
    ...
    if (result instanceof Promise) {
      return result
        ...
        .then((output) =>
          parseToInstance ? plainToInstance(model, output) : output,
        )
        .catch((error: Error) => {
    ...
    if (parseResult.success) {
      return parseToInstance
        ? plainToInstance(model, parseResult.data)
        : parseResult.data;
    }
