/* eslint-disable @typescript-eslint/no-explicit-any, @typescript-eslint/naming-convention, @typescript-eslint/no-use-before-define, @typescript-eslint/strict-boolean-expressions, @typescript-eslint/no-non-null-assertion */
...
export function InputSchema<T extends ZodTypeAny>(
  ...
  if (!isZodInstance(ZodObject, input)) {
    ...
    const RegisteredType = _getOrCreateRegisteredType(input as AnyZodObject, {
      getScalarTypeFor,
    });
  ...
  return function _anonymousInputSchemaWrapper(target, propKey, index) {
    options ??= {};
 ...
 */
export type InputSchemaOf<T extends ZodTypeAny> = Infer<T>;

/**
 * Frees the used objects during the startup.
 *
 * The {@link InputSchema} decorator uses helper local variables to keep the
 ...
 * @export
 */
export function freeInputSchema() {
