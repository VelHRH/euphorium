/* eslint-disable @typescript-eslint/no-explicit-any, @typescript-eslint/naming-convention, @typescript-eslint/strict-boolean-expressions, import/no-duplicates */
import { type BaseTypeOptions, GraphQLISODateTime } from '@nestjs/graphql';
...
type BaseOptions<T extends AnyZodObject> = WrapWithZodOptions<
  BaseTypeOptions,
  T
>;
...
/**
 * Default type provider that handles built-in scalar types.
 */
const DEFAULT_SCALAR_TYPE_PROVIDER: TypeProvider = (typeName: string) => {
  switch (typeName) {
    case 'Date':
...
export function getDefaultTypeProvider(): TypeProvider | undefined {
  return DEFAULT_TYPE_PROVIDER || DEFAULT_SCALAR_TYPE_PROVIDER;
}
