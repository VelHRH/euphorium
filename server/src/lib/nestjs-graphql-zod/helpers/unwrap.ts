...
export function unwrapNestedZod<T extends ZodType>(
  ...
  if (isZodInstance(ZodArray, input)) {
    return input.element as UnwrapNestedZod<T>;
  }

  if (isZodInstance(ZodCatch, input)) {
  ...
  if (isZodInstance(ZodEffects, input)) {
    return input.innerType() as UnwrapNestedZod<T>;
  }

  if (isZodInstance(ZodLazy, input)) {
  ...
  if (isZodInstance(ZodPromise, input)) {
    return input.unwrap() as UnwrapNestedZod<T>;
  }

  if (isZodInstance(ZodSet, input)) {
    return input._def.valueType as UnwrapNestedZod<T>;
  }
