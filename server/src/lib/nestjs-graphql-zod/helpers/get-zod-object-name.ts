...
export function getZodObjectName(instance: ZodTypeAny): string {
  ...
  if (isZodInstance(ZodBoolean, instance)) {
    return 'Boolean';
  }

  if (isZodInstance(ZodString, instance)) {
    return 'String';
  }

  if (isZodInstance(ZodNumber, instance)) {
    return 'Number';
  }

  if (isZodInstance(ZodBigInt, instance)) {
    return 'BigInt';
  }

  if (isZodInstance(ZodDate, instance)) {
    return 'Date';
  }

  if (isZodInstance(ZodAny, instance)) {
    return 'Any';
  }

  if (isZodInstance(ZodNull, instance)) {
    return 'Null';
  }
