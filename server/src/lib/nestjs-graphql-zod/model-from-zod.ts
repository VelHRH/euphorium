 ...
 * @param {ClassDecorator} decorator The decorator function.
 ...
 */
export function modelFromZodBase<T extends AnyZodObject, O extends Options<T>>(
  zodInput: T,
  ...
  const { keepZodObject: shouldKeepZodObject = false } = options;

  class DynamicZodModel {}

  const { prototype } = DynamicZodModel;
...
>(zodInput: T, options: O = {} as O): Type<TypeOf<T>> {
  ...
  return modelFromZodBase(zodInput, decorator, options);
}
